...
class SonarFOV():
    ...
    def build_vector_list(self):
        ...
        for elevation in self.sonar_msg_metadata.elevations:
            ce = np.cos(elevation)
            se = np.sin(elevation)
            ca = np.cos(self.sonar_msg_metadata.azimuths)
            sa = np.sin(self.sonar_msg_metadata.azimuths)

            z = self.sonar_msg_metadata.max_range * ce * ca
            y = -1 * self.sonar_msg_metadata.max_range * ce * sa
            x = np.full_like(z,
                             fill_value=self.sonar_msg_metadata.max_range * se)
            xx.extend(x)
            yy.extend(y)
            zz.extend(z)
